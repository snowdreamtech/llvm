...
Docker LLVM 模板提供了一个与 GCC `build-essential` 对标的 LLVM/Clang 工具链系统。它提供标准化的容器基础，具有灵活的入口点系统、多架构支持以及跨 Alpine、Debian 和 Rocky Linux 发行版的一致配置模式。

## 概述

Docker LLVM 模板作为构建 C/C++ 应用程序及使用 LLVM 编译器基础架构的起点。它提供：

- **完整的 LLVM/Clang 工具链**（llvm、clang、lld、lldb、compiler-rt、libc++）
- **标准的构建工具**（make、cmake、ninja、python3、gdb）
...
**支持架构**: amd64, arm32v7, arm64, ppc64le, riscv64, s390x
